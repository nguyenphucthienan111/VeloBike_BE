...
class OrderController {
    ...
    static startInspection(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a;
            try {
                const { id } = req.params;
                ...
                const order = yield Order_1.Order.findById(id).populate("listingId");
                if (!order) {
                    return res.status(404).json({ success: false, message: "Order not found" });
                }
                // Check if order is in correct status
                if (order.status !== Order_1.OrderStatus.ESCROW_LOCKED) {
                    return res.status(400).json({
                        success: false,
                ...
                order.inspectorId = finalInspectorId;
                yield order.save();
                yield OrderService_1.OrderService.startInspection(id, finalInspectorId);
                const updatedOrder = yield Order_1.Order.findById(id)
                    .populate("listingId")
                    .populate("buyerId", "fullName")
                    .populate("sellerId", "fullName")
                    .populate("inspectorId", "fullName");
